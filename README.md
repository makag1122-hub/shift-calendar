...
- **Firestore 보안 규칙**: [`firestore.rules`](firestore.rules) 파일 전체를 규칙 탭에 게시합니다. 공유방 구성원만 근무표와 참여자 닉네임을 읽을 수 있고, 각 사용자는 본인 이름 문서만 쓸 수 있습니다.
- **카카오 로그인(앱 전용)**: Kakao Developers에서 **카카오 로그인 사용 설정을 ON**으로 바꾸고, **동의항목 → 닉네임**을 설정합니다. 앱은 닉네임만 공유방에 저장하며 이메일·전화번호·프로필 사진은 기능에 이용하지 않습니다.
