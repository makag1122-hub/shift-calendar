...
- **명절 특별근무**: 설날·추석에는 휴무가 길게 편성돼 정규 패턴과 달라집니다. 조별 편성이 코드에 내장되어 자동 적용되며, 정규 패턴과 다른 날은 칸 **위쪽 보라색 줄**로 표시됩니다.
  - 2026 추석(9/24~26) 편성이 들어 있습니다. (A/B/C/D 각 조: 하루 근무 → 긴 휴무 → 하루 근무)
  - 명절 편성 탓에 생기는 **6일 이상 연속근무**는 칸 **아래쪽 주황색 줄**로 표시되고, 날짜를 누르면 “🔺 6일 연속근무 (오늘 N일차)” 안내가 나옵니다.
- 특정 날이 실제와 다르면 달력에서 그 날을 눌러 직접 바꿀 수 있고(수동 변경이 명절 편성보다 우선), ‘되돌리기’를 누르면 명절 편성 값으로 돌아옵니다.
- 다음 명절 편성은 `app.js`의 `SPECIAL_SCHEDULE`에 날짜만 이어서 추가하면 됩니다.
