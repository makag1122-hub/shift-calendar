...
- **Firestore 보안 규칙**: [`firestore.rules`](firestore.rules) 파일 전체를 규칙 탭에 게시합니다. 공유방 구성원만 근무표와 참여자 이름을 읽을 수 있고, 각 사용자는 본인 이름 문서만 쓸 수 있습니다.
- **내 이름 표시**: `설정 → 친구와 공유하기 → 내 이름 정하기`에서 이름을 직접 입력합니다. 로그인은 필요 없고, 입력한 이름은 이 기기(`localStorage`의 `shiftcal.profile`)에 저장된 뒤 공유방 참여자 목록과 초대 문구에 쓰입니다. 앱과 PWA 모두에서 동작합니다.
